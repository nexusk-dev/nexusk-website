...
import {BarChart3, Bell, ChevronRight, Monitor, Settings, Shield, Terminal, Users} from 'lucide-react';
    ...
    return (
        <div className="flex min-h-screen bg-nexus-darker text-nexus-text">
            ...
            <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
                <div
                    className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 bg-nexus-primary/5 rounded-full blur-3xl animate-pulse"/>
                <div
                    className="absolute -bottom-1/4 -right-1/4 w-1/2 h-1/2 bg-nexus-secondary/5 rounded-full blur-3xl animate-pulse delay-1000"/>
            </div>

            {/* 侧边栏 */}
            <aside
                className={`relative z-20 flex-shrink-0 bg-nexus-dark/50 backdrop-blur-md border-r border-nexus-primary/20 transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-64'}`}>
                <div className="flex flex-col h-full">
                    <div className="p-4 flex items-center justify-center h-20 border-b border-nexus-primary/20">
                        <Link href="/admin">
                            <Shield
                                className={`w-8 h-8 text-nexus-primary transition-all ${sidebarCollapsed ? 'mx-auto' : 'mr-3'}`}/>
                        </Link>
                        {!sidebarCollapsed && (
                            <HologramEffect>
                    ...
                    >
                        <ChevronRight
                            className={`w-6 h-6 mx-auto transition-transform ${sidebarCollapsed ? '' : 'rotate-180'}`}/>
                    </button>
                </div>
            </aside>
            ...
            <div className="flex-1 flex flex-col relative z-10">
                <header
                    className="bg-nexus-dark/50 backdrop-blur-md border-b border-nexus-primary/20 p-4 flex items-center justify-between h-20">
                    <div>
