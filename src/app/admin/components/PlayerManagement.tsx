...
import {CyberInput, GlassCard, NeonButton} from '@/components/ui';
import {PlayerHead} from '@/components/minecraft';
import {Ban, Eye, Filter, MessageSquare, Search, UserCheck, UserX} from 'lucide-react';
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <CyberInput icon={Search} placeholder="Search players..." value={searchTerm}
                ...
                <table className="w-full min-w-[800px]">
                    ...
                    <tr>
                        {['Player', 'Status', 'Rank', 'Playtime', 'Violations', 'Actions'].map(h => (
                            <th key={h}
                                className="p-4 text-left font-matrix text-sm text-nexus-primary uppercase tracking-wider">{h}</th>
                        ))}
                    </tr>
                    </thead>
                    <tbody>
                    {filteredPlayers.map(player => (
                        <tr key={player.id}
                            className="border-b border-nexus-surface hover:bg-nexus-surface/50 transition-colors">
                            <td className="p-4">
                            ...
                            <td><span
                                className={`text-sm font-matrix ${player.online ? 'text-nexus-accent' : 'text-nexus-text-muted'}`}>{player.online ? 'Online' : 'Offline'}</span>
                            </td>
                            <td><span
                                className={`text-xs px-2 py-1 rounded font-matrix font-bold ${rankConfig[player.rank].className}`}>{player.rank}</span>
                            </td>
                            <td><span className="font-matrix text-white">{player.playtime}</span></td>
                            <td><span
                                className={`text-xs px-2 py-1 rounded font-matrix font-bold ${violationConfig(player.violations)}`}>{player.violations}</span>
                            </td>
                            <td className="p-4">
                                <div className="flex items-center space-x-2">
                                    <button className="text-nexus-primary hover:text-white p-1"><Eye
                                        className="w-4 h-4"/></button>
                                    <button className="text-nexus-secondary hover:text-white p-1"><MessageSquare
                                        className="w-4 h-4"/></button>
                                    <button className="text-nexus-warning hover:text-white p-1"><UserX
                                        className="w-4 h-4"/></button>
                                    <button className="text-nexus-error hover:text-white p-1"><Ban className="w-4 h-4"/>
                                    </button>
                                </div>
                            </td>
                        </tr>
