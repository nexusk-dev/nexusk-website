...
import {Check, Copy, RefreshCw, Server, Users, Zap} from 'lucide-react'
...
export default function ServerStatus() {
    ...
    return (
        <div className={`glass-card border-${statusColor}/50 relative overflow-hidden`}>
            ...
            {serverInfo?.online && (
                <div
                    className="absolute inset-0 bg-gradient-to-r from-nexus-accent/5 to-transparent animate-pulse"></div>
            )}
            ...
            <div className="relative">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        <div
                            className={`w-4 h-4 rounded-full bg-${statusColor} ${serverInfo?.online ? 'animate-pulse' : ''}`}></div>
                        <h3 className="font-cyber text-xl text-nexus-text">
                            {serverInfo?.serverName || 'NexusK'}
                        </h3>
                        <span
                            className={`px-3 py-1 rounded-full text-xs font-matrix bg-${statusColor}/20 text-${statusColor} border border-${statusColor}/30`}>
    {statusText}
    </span>
                    </div>
