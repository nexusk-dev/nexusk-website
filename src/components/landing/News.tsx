...
const newsItems = [
    { type: 'UPDATE', title: 'Server v2.1.0 Released', content: 'New cyberpunk buildings and enhanced mod compatibility.', priority: 'high' as const },
    { type: 'EVENT', title: 'Cyberpunk Building Contest', content: 'Show off your futuristic architectural skills! Prizes available.', priority: 'medium' as const },
    { type: 'NOTICE', title: 'Scheduled Maintenance', content: 'Server will restart for performance optimization on Sunday.', priority: 'low' as const },
];
                        ...
                        <GlassCard hover={false} className="h-full">
                            <div className="space-y-4">
                                {newsItems.map((item, index) => {
                                    ...
                                    return (
                                        <div key={index} className={`bg-nexus-darker/50 rounded-lg p-4 border-l-4 transition-all hover:bg-nexus-surface ${config.className}`}>
                                            <span className={`text-xs px-2 py-1 rounded font-matrix font-bold uppercase mb-2 inline-block ${config.bgClassName}`}>{item.type}</span>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {events.map((event, index) => (
                                        <div key={index} className="bg-nexus-darker/50 rounded-md p-3 border-l-2 border-nexus-accent">
